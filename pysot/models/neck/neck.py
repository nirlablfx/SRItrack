...
class AdjustAllLayer(nn.Module):
    def __init__(self, in_channels, out_channels):
        ...
        if self.num == 1:
            self.downsample = AdjustLayer(in_channels[0], out_channels[0])  #降维的1x1卷积
        else:
            for i in range(self.num):
                self.add_module('downsample'+str(i+2),
                                AdjustLayer(in_channels[i], out_channels[i])) #输入通道不同，对应不同层的特征图
                                                                              #512是layer2 1024是layer3 2048是layer4
...
class FeatureEnhance(nn.Module):
    def __init__(self,
        ...
        for i in range(3):
            ...
            self.deform_convs.append(DeformConv2d())
